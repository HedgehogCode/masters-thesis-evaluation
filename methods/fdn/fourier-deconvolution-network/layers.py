...
def psf2otf(psf, img_shape):
    ...
    psf_shape = tf.shape(input=psf)
    psf_type = psf.dtype

    # coordinates for 'cutting up' the psf tensor
    midH = tf.math.floordiv(psf_shape[0], 2)
    midW = tf.math.floordiv(psf_shape[1], 2)
    ...
    otf = tf.signal.fft2d(tf.cast(tf.transpose(a=pre_otf, perm=[2,3,0,1]), tf.complex64))
    ...
    def call(self, x, mask=None):
        if self.mode == 'REPLICATE':
            ...
            for i in range(self.border):
                x = tf.pad(tensor=x, paddings=[[0,0], [1,1], [1,1], [0,0]], mode="SYMMETRIC")
        elif self.mode == 'ZEROS':
            x = tf.pad(tensor=x, paddings=[[0,0], [self.border, self.border], [self.border, self.border], [0,0]], mode="CONSTANT")
        return x
...
class FourierDeconvolution(Layer):
    ...
    def build(self, input_shapes):
        ...
        self._trainable_weights = [self.filter_weights]
    ...
    def call(self, inputs, mask=None):
        ...
        imagesize = tf.shape(input=padded_inputs)[1:3]
        kernelsize = tf.shape(input=blur_kernels)[1:3]
        padding = tf.math.floordiv(kernelsize,2)

        mask_int = tf.ones((imagesize[0]-2*padding[0],imagesize[1]-2*padding[1]), dtype=tf.float32)
        mask_int = tf.pad(tensor=mask_int, paddings=[[padding[0],padding[0]],[padding[1],padding[1]]], mode='CONSTANT')
        ...
        otf_term = tf.reduce_sum(input_tensor=tf.square(tf.abs(filter_otfs)), axis=1)

        k = tf.expand_dims(tf.transpose(a=blur_kernels, perm=[1,2,0]), -1)
        k_otf = psf2otf(k, imagesize)[:,0,:,:]

        if self.stage > 1:
            # boundary adjustment
            Kx_fft       = tf.signal.fft2d(tf.cast(padded_inputs[:,:,:,0], tf.complex64)) * k_otf
            Kx           = tf.cast(tf.signal.ifft2d(Kx_fft), dtype=tf.float32)
            Kx_outer     = (1.0 - mask_int) * Kx
            y_inner      = mask_int * observations[:,:,:,0]
            y_adjusted   = y_inner + Kx_outer
            dataterm_fft = tf.signal.fft2d(tf.cast(y_adjusted, tf.complex64)) * tf.math.conj(k_otf)
        else:
            # standard data term
            observations_fft = tf.signal.fft2d(tf.cast(observations[:,:,:,0], tf.complex64))
            dataterm_fft = observations_fft * tf.math.conj(k_otf)

        lambdas = tf.expand_dims(lambdas, -1)

        adjustment_fft = tf.signal.fft2d(tf.cast(adjustments[:,:,:,0], tf.complex64))
        ...
        return tf.expand_dims(tf.cast(tf.signal.ifft2d(frac_fft), dtype=tf.float32), -1)