...
def psnr(imgs_a, imgs_b):
    return tf.image.psnr(imgs_a, imgs_b, max_val=1).numpy()


@_handle_unbatched_inputs
def ssim(imgs_a, imgs_b):
    return tf.image.ssim(imgs_a, imgs_b, max_val=1).numpy()
...
def fsim(imgs_a, imgs_b):
    ...
    return tf.map_fn(fsim_on_stacked_image, stacked).numpy()  # type: ignore
...
def lpips_alex(imgs_a, imgs_b):
    ...
    return lpips_model_alex([imgs_a, imgs_b]).numpy()
...
def lpips_vgg(imgs_a, imgs_b):
    ...
    return lpips_model_vgg([imgs_a, imgs_b]).numpy()
...
def init_lpips_model_alex():
    ...
    lpips_model_alex = tf.keras.models.load_model(model_file)
...
def init_lpips_model_vgg():
    ...
    lpips_model_vgg = tf.keras.models.load_model(model_file)


# A dictionary of all metrics defined here
METRICS = {
    "PSNR": psnr,
    "SSIM": ssim,
    "FSIM": fsim,
